...
class solution{
	public static void printstring(Map<Integer, ArrayList> map, int start, String s, String sentence, List<String> output){
			...
			while(iter.hasNext()){
				...
				if(i == 0){
					...
					temp = sentence +  " " + temp;
          			...
          			if(end < s.length()){
          					printstring(map, end, s, temp, output);
          			}else{
          				output.add(temp);
          			}
			}
